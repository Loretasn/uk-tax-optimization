...
class SimpleGA:
    def __init__(self, pop_size=40):
        ...
        self.mutation = 0.15
        self.low = np.array([12000, 45000, 100000, 0.15, 0.30, 0.40])
        self.high = np.array([20000, 60000, 130000, 0.24, 0.44, 0.50])
...
for epsilon in EPSILONS:
    for seed in SEEDS:
        ...
        try:
            ...
            print(f"\nOptimal Policy:")
            print(f"  Personal Allowance: £{best[0]:,.0f}")
            print(f"  Basic Rate Threshold: £{best[1]:,.0f}")
            print(f"  Higher Rate Threshold: £{best[2]:,.0f}")
            print(f"  Basic Rate: {best[3]*100:.1f}%")
            print(f"  Higher Rate: {best[4]*100:.1f}%")
            print(f"  Additional Rate: {best[5]*100:.1f}%")
